...
export function AppProvider({ children }) {
    ...
    const apiActions = {
        ...
        deleteMember: async (id) => {
            ...
            dispatch({ type: ACTIONS.DELETE_MEMBER, payload: id });
        },
        ...
        addPhase: async (phase) => {
            const { data } = await configApi.createPhase(phase);
            dispatch({ type: ACTIONS.ADD_PHASE, payload: data });
            return data;
        },
        updatePhase: async (id, phase) => {
            const { data } = await configApi.updatePhase(id, phase);
            dispatch({ type: ACTIONS.UPDATE_PHASE, payload: data });
            return data;
        },
        deletePhase: async (id) => {
            await configApi.deletePhase(id);
            dispatch({ type: ACTIONS.DELETE_PHASE, payload: id });
        },
        addTask: async (task) => {
            const { data } = await configApi.createTask(task);
            dispatch({ type: ACTIONS.ADD_TASK, payload: data });
            return data;
        },
        updateTask: async (id, task) => {
            const { data } = await configApi.updateTask(id, task);
            dispatch({ type: ACTIONS.UPDATE_TASK, payload: data });
            return data;
        },
        deleteTask: async (id) => {
            await configApi.deleteTask(id);
            dispatch({ type: ACTIONS.DELETE_TASK, payload: id });
        },
        addCostCenter: async (cc) => {
            const { data } = await financeApi.createCostCenter(cc);
            dispatch({ type: ACTIONS.ADD_COST_CENTER, payload: data });
            return data;
        },
        updateCostCenter: async (id, cc) => {
            const { data } = await financeApi.updateCostCenter(id, cc);
            dispatch({ type: ACTIONS.UPDATE_COST_CENTER, payload: data });
            return data;
        },
        deleteCostCenter: async (id) => {
            await financeApi.deleteCostCenter(id);
            dispatch({ type: ACTIONS.DELETE_COST_CENTER, payload: id });
        },
        addCOA: async (coa) => {
            const { data } = await financeApi.createCOA(coa);
            dispatch({ type: ACTIONS.ADD_COA, payload: data });
            return data;
        },
        updateCOA: async (id, coa) => {
            const { data } = await financeApi.updateCOA(id, coa);
            dispatch({ type: ACTIONS.UPDATE_COA, payload: data });
            return data;
        },
        deleteCOA: async (id) => {
    ...
    return (
        <AppContext.Provider value={{ state, dispatch, ACTIONS, ...apiActions }}>
            {children}
        </AppContext.Provider>
    );
}
