...
// --- Validation Functions moved outside component to resolve Rollup binding issues ---
const validateCostCenterCode = (code, existingCostCenters = [], excludeId = null) => {
    ...
    if (trimmedCode.length < 2) errors.push('Code must be at least 2 characters long');
    if (trimmedCode.length > 10) errors.push('Code must not exceed 10 characters');
    ...
    if (duplicate) errors.push('Code must be unique');
    return errors;
};
...
const validateCostCenterName = (name) => {
    ...
    if (reservedWords.some(word => upperName.includes(word))) {
        errors.push('Name cannot contain reserved words');
    }
    return errors;
};
...
const validateCostCenterDescription = (description) => {
    ...
    if (trimmedDescription.length > 500) errors.push('Description must not exceed 500 characters');
...
const validateCostCenterManager = (manager) => {
    const errors = [];
    ...
    if (trimmedManager.length < 2) errors.push('Manager name too short');
    if (trimmedManager.length > 100) errors.push('Manager name too long');
    if (!/^[a-zA-Z\s\-'.]+$/.test(trimmedManager)) {
        errors.push('Manager name contains invalid characters');
    }
    return errors;
};
...
const validateCostCenterBudget = (monthlyBudget, yearlyBudget) => {
    ...
    if (monthlyBudget !== undefined && monthlyBudget !== null && monthlyBudget !== '') {
        ...
        if (isNaN(monthly) || monthly < 0) errors.push('Monthly budget must be a positive number');
    }
    if (yearlyBudget !== undefined && yearlyBudget !== null && yearlyBudget !== '') {
        const yearly = Number(yearlyBudget);
        if (isNaN(yearly) || yearly < 0) errors.push('Yearly budget must be a positive number');
    }
    return errors;
};
...
const validateBudgetPeriod = (budgetPeriod) => {
    const errors = [];
    if (!budgetPeriod) return errors;
    const trimmedPeriod = budgetPeriod.trim();
    if (!/^\d{4}$/.test(trimmedPeriod)) errors.push('Budget period must be a 4-digit year');
    return errors;
};
...
const CostCenterForm = memo(({
    ...
    return (
        <DialogContent className="sm:max-w-2xl max-h-[90vh] bg-card border-border shadow-2xl p-0 overflow-hidden flex flex-col rounded-2xl">
            ...
            <DialogFooter className="p-6 bg-muted/10 border-t border-border mt-0 gap-2 sm:gap-0 flex-shrink-0">
                <Button
                    variant="ghost"
                    onClick={onCancel}
...
export default function CostCenters() {
    const { state, dispatch, addCostCenter, updateCostCenter, deleteCostCenter } = useApp();
    ...
    const columns = useMemo(() => [
        {
            ...
            header: "ID",
            width: 70,
            render: (item) => <span className="text-[10px] font-mono text-slate-400 font-bold">{item.id.replace('CC-', '')}</span>,
        },
        {
            key: "code",
            header: "Code",
            width: 80,
            render: (item) => <span className="font-black text-slate-900 dark:text-slate-100 font-mono text-xs">{item.code}</span>,
        },
        {
            key: "name",
            header: "Name",
            minWidth: 160,
            render: (item) => <span className="font-bold text-slate-900 dark:text-slate-100 truncate block">{item.name}</span>,
        },
        {
            key: "parent",
            header: "Parent",
            ...
            render: (item) => {
                ...
                return parent ? (
                    <Badge variant="outline" className="font-mono text-[9px] border-slate-200 text-slate-500 bg-slate-50">
                        {parent.code}
                    </Badge>
        ...
        },
        {
            ...
            header: "Manager",
            width: 120,
            render: (item) => <span className="text-[11px] font-bold text-slate-600 dark:text-slate-400 truncate block">{item.manager}</span>,
        },
        {
            key: "budget",
            header: "Budget",
            ...
            render: (item) => (
                <span className="text-xs font-bold text-slate-500 font-mono">
                    {item.monthlyBudget ? `${(item.monthlyBudget / 1000000).toFixed(0)}M` : '—'}
                </span>
            ),
        },
        {
            ...
            render: (item) => (
                <span className="text-xs font-black text-indigo-600 dark:text-indigo-400 font-mono">
                    {item.actualMonthlyCost ? `${(item.actualMonthlyCost / 1000000).toFixed(1)}M` : '0'}
                </span>
            ),
        },
        {
            ...
            width: 80,
            render: (item) => {
                if (!item.monthlyBudget) return null;
                const variance = item.monthlyBudget - (item.actualMonthlyCost || 0);
                const variancePercent = (variance / item.monthlyBudget) * 100;
                const isOver = variance < 0;
                return (
                    <div className={cn(
                        "flex items-center justify-center gap-1.5 text-[10px] font-black px-2 py-0.5 rounded-md",
                        isOver ? "text-rose-600 bg-rose-50" : "text-emerald-600 bg-emerald-50"
                    )}>
                        <span className="uppercase tracking-tighter opacity-70">
                            {isOver ? "Over" : "Under"}
                        </span>
                        <span className="font-mono">
                            {isOver ? '-' : '+'}{Math.abs(variancePercent).toFixed(0)}%
                        </span>
                    </div>
                );
            },
        },
        {
            ...
            render: (item) => (
                <div className="flex items-center gap-2">
                    <Badge className={cn(
                        "rounded-full px-2 py-0 text-[10px] uppercase font-black border-none h-4",
                        item.isActive ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-500"
                    )}>
                        {item.isActive ? "Active" : "Inactive"}
                    </Badge>
                    <span className="text-[10px] font-bold text-slate-400 font-mono">{item.budgetPeriod}</span>
                </div>
            ),
        },
        {
            ...
            render: (item) => (
                <div className="flex items-center justify-end">
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-300 hover:text-indigo-600" onClick={() => handleEdit(item)}>
                        <Edit2 className="h-3.5 w-3.5" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-300 hover:text-rose-600" onClick={() => handleDeleteClick(item)}>
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (editingCostCenter) {
                await updateCostCenter(formData.id, formData);
            ...
            if (error.message.includes('Code must be unique') || error.message.includes('already in use')) {
    ...
    return (
        <CostCenterErrorBoundary componentName="CostCenters" showErrorDetails={import.meta.env.MODE === 'development'}>
            <div className="library-page space-y-6 animate-in fade-in duration-500">
